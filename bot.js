...
function generateRuleBasedAnalytics(titleEn, descEn) {
  ...
  const enLines = top.map((imp) => `${imp.dir} ${imp.asset} — ${imp.reason_en || imp.reason}`);
  ...
  if (process.env.GEMINI_API_KEY) {
    try {
      ...
      const result = await model.generateContent(
        `Переведи на русский язык. Верни ТОЛЬКО перевод, без пояснений и кавычек:\n${text}`
      );
...
async function postSingleTweet(text, replyToId = null) {
  if (!X_API_KEY || !X_API_SECRET || !X_ACCESS_TOKEN || !X_ACCESS_TOKEN_SECRET) {
    console.log("[X] Keys not configured, skipping");
    return null;
  }
  try {
    ...
    if (res.ok) {
      ...
      console.log(`[${new Date().toISOString()}] [X] Tweet posted: ${id}`);
      return id;
    } else {
...
async function main() {
  ...
  await sendTelegram(
    ...
    "🐦 X: 2 твита — новость + аналитика на английском\n" +
