...
// ─── Translation (Google Translate primary, Gemini fallback) ─────────────────
...
async function translateToRussian(text) {
  ...
  // 2. Gemini (запасной — если Google Translate недоступен с сервера)
  if (process.env.GEMINI_API_KEY) {
    try {
      const model = genAI.getGenerativeModel({ model: "gemini-2.0-flash" });
      const result = await model.generateContent(
        `Translate to Russian. Return ONLY the translation, no explanations, no quotes:\n${text}`
      );
...
const PRICE_ASSETS = [
  ...
  { symbol: "HG=F",      label: "Медь (Copper)",             emoji: "🔶", unit: "$/lb", roundTo: 0.50,  decimals: 2, lastLevel: null, lastAlertTime: null }, // 4.0→4.5→5.0
  // ── Сельхоз товары ───────────────────────────────────────────────────────
