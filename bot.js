...
// ─── Ультра-краткий словарь триггеров влияния на активы ───────────────────────
const IMPACT_DICTIONARY = [
  ...
  {
    keywords: [["trump", "tariffs", "trade war", "трамп", "тарифы", "пошлины"]],
    impact: "🇺🇸 Торговые войны — доллар (USD) сильнее, давление на евро (EUR) и юань (CNY)."
  },
  {
    keywords: [["nfp", "unemployment", "jobs", "занятость", "безработица"], ["bad", "weak", "rise", "плох", "слаб", "вырос"]],
    impact: "📉 Слабый рынок труда США — давление на USD, позитив для золота."
  },
  {
    keywords: [["nfp", "unemployment", "jobs", "занятость", "безработица"], ["good", "strong", "fall", "хорош", "крепк", "упал"]],
    impact: "💪 Сильный рынок труда США — доллар (USD) укрепляется."
  }
];
...
function currentMoscowDateKey() { return new Date().toLocaleDateString("en-CA", { timeZone: "Europe/Moscow" }); }
function moscowDayOfWeek() { return new Date(new Date().toLocaleString("en-US", { timeZone: "Europe/Moscow" })).getDay(); }
...
async function sendTelegram(text) {
  ...
  await Promise.all([sendToChat(TELEGRAM_CHAT_ID, text), sendToChat(TELEGRAM_CHANNEL, text)]);
}
...
async function checkFeed(feedUrl) {
  try {
    ...
    for (const entry of feed.items.slice(0, 10)) {
      ...
      if (pubDate && now - pubDate > MAX_ARTICLE_AGE_MS) { sentArticles.add(titleEn); continue; }
      const cleanDesc = descEn.replace(/\s*This article was written by .+?\./gi, "").replace(/\s*Read more at .+?\./gi, "").trim();
      ...
      const [titleRu, descRu] = await Promise.all([translateToRussian(titleEn), translateToRussian(shortDesc)]);
      ...
      const message = (`${header}\n\n📌 *${titleRu}*\n${descBlock}${impactBlock}🇬🇧 _Оригинал:_\n_${shortTitleEn}_\n\n🔗 [Источник](${link})`).slice(0, 4000);
      await sendTelegram(message);
      await new Promise((r) => setTimeout(r, MSG_SEND_DELAY_MS));
    }
  } catch (err) { console.error(err); }
}
async function checkAllFeeds() { for (const feed of FEEDS) await checkFeed(feed.url); }
async function checkCalendarReminders() {
  try {
    ...
    for (const event of events) {
      ...
      const timeStr = new Date(event.date).toLocaleTimeString("ru-RU", { hour: "2-digit", minute: "2-digit", timeZone: "Europe/Moscow" });
      const titleRu = await translateToRussian(event.title);
      await sendTelegram(`⏰ *ЧЕРЕЗ 15 МИНУТ | ВЫСОКИЙ ПРИОРИТЕТ*\n\n${flag} *${titleRu}*\n🇬🇧 _${event.title}_\n\n${event.forecast ? `📊 Прогноз: *${event.forecast}*\n` : ""}${event.previous ? `📉 Пред. значение: ${event.previous}\n` : ""}🕐 Время выхода: *${timeStr} МСК*`);
    }
  } catch (err) { console.error(err); }
}

const PRICE_ASSETS = [
  { symbol: "GC=F", label: "Золото (XAU/USD)", emoji: "🥇", unit: "$", levels: [{ price: 4400, direction: "below", alerted: false }, { price: 4500, direction: "below", alerted: false }, { price: 4600, direction: "above", alerted: false }, { price: 4700, direction: "above", alerted: false }, { price: 4800, direction: "above", alerted: false }, { price: 5000, direction: "above", alerted: false }] },
  { symbol: "CL=F", label: "Нефть WTI", emoji: "🛢️", unit: "$", levels: [{ price: 55, direction: "below", alerted: false }, { price: 60, direction: "below", alerted: false }, { price: 70, direction: "above", alerted: false }, { price: 80, direction: "above", alerted: false }, { price: 90, direction: "above", alerted: false }, { price: 100, direction: "above", alerted: false }] },
  { symbol: "BZ=F", label: "Нефть Brent", emoji: "⛽", unit: "$", levels: [{ price: 60, direction: "below", alerted: false }, { price: 65, direction: "below", alerted: false }, { price: 75, direction: "above", alerted: false }, { price: 85, direction: "above", alerted: false }, { price: 95, direction: "above", alerted: false }, { price: 105, direction: "above", alerted: false }] },
  { symbol: "DX-Y.NYB", label: "Индекс доллара (DXY)", emoji: "💵", unit: "pts", levels: [{ price: 98, direction: "below", alerted: false }, { price: 100, direction: "below", alerted: false }, { price: 105, direction: "above", alerted: false }, { price: 108, direction: "above", alerted: false }, { price: 110, direction: "above", alerted: false }] }
];
...
async function initPriceLevels() {
  for (const asset of PRICE_ASSETS) {
    ...
    for (const level of asset.levels) {
      if (level.direction === "above" ? price >= level.price : price <= level.price) level.alerted = true;
    }
  }
}
...
async function checkPriceAlerts() {
  for (const asset of PRICE_ASSETS) {
    ...
    for (const level of asset.levels) {
      ...
      const reversed = level.direction === "above" ? price < level.price - resetBuffer : price > level.price + resetBuffer;
      if (level.alerted && reversed) { level.alerted = false; continue; }
      if (!level.alerted && crossed) {
        level.alerted = true;
        const sign = level.direction === "above" ? "выше" : "ниже";
        await sendTelegram(`${level.direction === "above" ? "📈" : "📉"} *ЦЕНОВОЙ АЛЕРТ*\n\n${asset.emoji} *${asset.label}*\nЦена *${sign} ${level.price} ${asset.unit}*\nТекущая цена: *${price.toFixed(2)} ${asset.unit}*`);
      }
    }
  }
}
...
function sessionWarnHHMM(session) {
  ...
  return { hour: Math.floor(((totalMin % 1440) + 1440) % 1440 / 60), minute: ((totalMin % 60) + 60) % 60 };
}

function sessionOpenTimeMoscow(session) {
  const { hour: curH, minute: curM } = getLocalHHMM(session.timezone);
  const diffMs = (session.openHour * 60 + session.openMinute - (curH * 60 + curM)) * 60_000;
  return new Date(Date.now() + diffMs).toLocaleTimeString("ru-RU", { hour: "2-digit", minute: "2-digit", timeZone: "Europe/Moscow" });
}
...
async function checkSessionAlerts() {
  ...
  for (const session of SESSIONS) {
    ...
    if (hour !== warn.hour || minute !== warn.minute || new Date().getSeconds() >= 45) continue;
    const alertKey = `${session.name}|${nowDateKey}`;
    if (sentSessionAlerts.has(alertKey)) continue;
    sentSessionAlerts.add(alertKey);
    await sendTelegram(`🔔 *ОТКРЫТИЕ СЕССИИ ЧЕРЕЗ ${session.warnMinutes} МИНУТ*\n\n${session.emoji} *${session.name}*\n🕐 Открытие: *${sessionOpenTimeMoscow(session)} МСК* (${String(session.openHour).padStart(2, "0")}:${String(session.openMinute).padStart(2, "0")} местного время)\n📍 _Учитывает летнее/зимнее время автоматически_`);
  }
}
...
async function sendWeeklyDigest() {
  try {
    ...
    if (highEvents.length === 0) { await sendTelegram("📅 *ДАЙДЖЕСТ НА НЕДЕЛЮ*\n\nНа этой неделе событий высокого приоритета не запланировано."); return; }
    const byDay = new Map();
    for (const event of highEvents) {
      const dayKey = new Date(event.date).toLocaleDateString("ru-RU", { weekday: "long", day: "numeric", month: "long", timeZone: "Europe/Moscow" });
    ...
    for (const [day, dayEvents] of byDay) {
      ...
      for (const event of dayEvents) {
        body += `  ${CURRENCY_FLAG[event.country] ?? "🌐"} ${new Date(event.date).toLocaleTimeString("ru-RU", { hour: "2-digit", minute: "2-digit", timeZone: "Europe/Moscow" })} МСК — *${event.title}*${event.forecast ? ` · Прогноз: ${event.forecast}` : ""}${event.previous ? ` · Пред: ${event.previous}` : ""}\n`;
      }
    }
...
async function sendDailyDigest() {
  try {
    ...
    const todayHigh = events.filter((e) => e.impact === "High" && new Date(e.date).toLocaleDateString("en-CA", { timeZone: "Europe/Moscow" }) === todayKey);
    if (todayHigh.length === 0) { await sendTelegram("🌅 *СВОДКА НА СЕГОДНЯ*\n\nСегодня событий высокого приоритета не запланировано."); return; }
    let body = "";
    for (const event of todayHigh) {
      body += `\n${CURRENCY_FLAG[event.country] ?? "🌐"} *${new Date(event.date).toLocaleTimeString("ru-RU", { hour: "2-digit", minute: "2-digit", timeZone: "Europe/Moscow" })} МСК* — ${event.title}${event.forecast ? ` · Прогноз: *${event.forecast}*` : ""}${event.previous ? ` · Пред: ${event.previous}` : ""}`;
    }
    const label = new Date().toLocaleDateString("ru-RU", { weekday: "long", day: "numeric", month: "long", timeZone: "Europe/Moscow" });
    await sendTelegram((`🌅 *СВОДКА НА СЕГОДНЯ | ${label.charAt(0).toUpperCase() + label.slice(1)}*\n📌 Высокоприоритетные события:\n${body}`).slice(0, 4000));
  } catch (err) { console.error(err); }
}
