...
const ANALYTICS_RULES = [
  {
    ...
    impacts: [
      { asset: "🔶 Медь (Copper)", dir: "↓", reason: "Китай — ~55% мирового спроса на медь; слабые данные давят на цену", reason_en: "China ~55% of global copper demand; weak data pressures price" },
  ...
  },
  {
    ...
    impacts: [
      { asset: "🔶 Медь (Copper)", dir: "↑", reason: "Рост промпроизводства Китая → спрос на медь растёт", reason_en: "China industrial growth → copper demand rises" },
  ...
  },
  {
    ...
    negKeywords: [],
    impacts: [
      { asset: "🔶 Медь (Copper) / Сталь / Алюминий", dir: "↑↓", reason: "Тарифы прямо влияют на металлы через цепочки поставок", reason_en: "Tariffs directly hit metals through supply chains" },
...
// Только Google Translate — используется для заголовка и текста новости
async function translateGT(text) {
  ...
  return text; // оригинал если Google Translate недоступен
}

// Gemini primary, Google Translate fallback — используется для экономического календаря и прочего
async function translateToRussian(text) {
  ...
  if (process.env.GEMINI_API_KEY) {
    try {
      ...
      const result = await model.generateContent(
        `Переведи следующий текст на русский язык. Верни ТОЛЬКО русский перевод, без пояснений, без кавычек, без оригинала:\n${text}`
      );
      const translated = result.response.text().trim();
      // Проверяем что ответ реально на русском (содержит кириллицу)
      if (translated && translated.length > 3 && /[а-яёА-ЯЁ]/.test(translated)) {
        return translated;
      }
    } catch { /* continue */ }
  }

  // 2. Google Translate (запасной)
  for (const clientId of ["gtx", "dict-chrome-ex"]) {
    try {
      const url = `https://translate.googleapis.com/translate_a/single?client=${clientId}&sl=en&tl=ru&dt=t&q=${encodeURIComponent(text)}`;
      const res = await fetch(url, {
  ...
  return text; // крайний случай: оригинал
}
...
async function checkFeed(feedUrl, strict = false) {
  try {
    ...
    for (const entry of feed.items.slice(0, 10)) {
      ...
      // Translate title and description to Russian for Telegram (Google Translate only)
      const [titleRu, descRu] = await Promise.all([
        translateGT(titleEn),
        translateGT(shortDesc),
      ]);
...
const PRICE_ASSETS = [
  // ── Драгоценные металлы ──────────────────────────────────────────────────
  { symbol: "GC=F",      label: "Золото (XAU/USD)",          emoji: "🥇", unit: "$",    roundTo: 100,   decimals: 0, lastLevel: null, lastAlertTime: null }, // 3000→3100→3200
  { symbol: "SI=F",      label: "Серебро (XAG/USD)",         emoji: "🥈", unit: "$",    roundTo: 5,     decimals: 0, lastLevel: null, lastAlertTime: null }, // 70→75→80→85→90
  { symbol: "PL=F",      label: "Платина (XPT/USD)",         emoji: "⬜", unit: "$",    roundTo: 50,    decimals: 0, lastLevel: null, lastAlertTime: null }, // 950→1000→1050
  { symbol: "PA=F",      label: "Палладий (XPD/USD)",        emoji: "🔘", unit: "$",    roundTo: 100,   decimals: 0, lastLevel: null, lastAlertTime: null }, // 900→1000→1100
  // ── Энергетика ───────────────────────────────────────────────────────────
  { symbol: "CL=F",      label: "Нефть WTI",                 emoji: "🛢️", unit: "$",    roundTo: 5,     decimals: 1, lastLevel: null, lastAlertTime: null }, // 60→65→70→75
  { symbol: "BZ=F",      label: "Нефть Brent",               emoji: "⛽",  unit: "$",    roundTo: 5,     decimals: 1, lastLevel: null, lastAlertTime: null }, // 65→70→75→80
  { symbol: "NG=F",      label: "Природный газ (Henry Hub)", emoji: "⚡", unit: "$",    roundTo: 0.50,  decimals: 2, lastLevel: null, lastAlertTime: null }, // 3.0→3.5→4.0
  // ── Промышленные металлы ─────────────────────────────────────────────────
  { symbol: "HG=F",      label: "Медь (Copper)",             emoji: "🔶", unit: "$/t",  roundTo: 500,   decimals: 0, multiplier: 2204.62, lastLevel: null, lastAlertTime: null }, // 9000→9500→10000
  // ── Сельхоз товары ───────────────────────────────────────────────────────
  { symbol: "ZW=F",      label: "Пшеница",                   emoji: "🌾", unit: "¢",    roundTo: 50,    decimals: 0, lastLevel: null, lastAlertTime: null }, // 500→550→600
  { symbol: "ZC=F",      label: "Кукуруза",                  emoji: "🌽", unit: "¢",    roundTo: 50,    decimals: 0, lastLevel: null, lastAlertTime: null }, // 400→450→500
  { symbol: "ZS=F",      label: "Соя",                       emoji: "🫘", unit: "¢",    roundTo: 100,   decimals: 0, lastLevel: null, lastAlertTime: null }, // 1000→1100→1200
  // ── Форекс ───────────────────────────────────────────────────────────────
  { symbol: "DX-Y.NYB",  label: "Индекс доллара (DXY)",      emoji: "💵", unit: "pts",  roundTo: 1,     decimals: 2, lastLevel: null, lastAlertTime: null }, // 103→104→105
  { symbol: "EURUSD=X",  label: "EUR/USD",                   emoji: "🇪🇺", unit: "",    roundTo: 0.01,  decimals: 4, lastLevel: null, lastAlertTime: null }, // 1.08→1.09→1.10
  { symbol: "GBPUSD=X",  label: "GBP/USD",                   emoji: "🇬🇧", unit: "",    roundTo: 0.01,  decimals: 4, lastLevel: null, lastAlertTime: null }, // 1.26→1.27→1.28
  { symbol: "JPY=X",     label: "USD/JPY",                   emoji: "🇯🇵", unit: "¥",   roundTo: 5,     decimals: 2, lastLevel: null, lastAlertTime: null }, // 150→155→160
  { symbol: "AUDUSD=X",  label: "AUD/USD",                   emoji: "🇦🇺", unit: "",    roundTo: 0.01,  decimals: 4, lastLevel: null, lastAlertTime: null }, // 0.63→0.64→0.65
  { symbol: "CADUSD=X",  label: "USD/CAD",                   emoji: "🇨🇦", unit: "",    roundTo: 0.01,  decimals: 4, lastLevel: null, lastAlertTime: null }, // 1.37→1.38→1.39
  // ── Облигации ────────────────────────────────────────────────────────────
  { symbol: "^TNX",      label: "US 10Y Yield",              emoji: "📉", unit: "%",    roundTo: 0.25,  decimals: 2, lastLevel: null, lastAlertTime: null }, // 4.00→4.25→4.50→4.75
];
...
async function initPriceLevels() {
  for (const asset of PRICE_ASSETS) {
    const raw = await fetchPrice(asset.symbol);
    if (raw === null) continue;
    const price = raw * (asset.multiplier ?? 1);
