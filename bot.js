...
async function translateToRussian(text) {
  ...
  for (let attempt = 0; attempt < 3; attempt++) {
    ...
    try {
      if (attempt < 2) {
        ...
        // HTML-эндпоинт как крайний запасной вариант
        const res = await fetch(url, {
          headers: { "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64)" },
          signal: AbortSignal.timeout(7000),
        });
  ...
  // Если перевод не удался — возвращаем оригинал и логируем
...
// silentMode = true на старте: заполняем sentArticles без отправки сообщений
async function checkFeed(feedUrl, strict = false, silentMode = false) {
  try {
    ...
    for (const entry of feed.items.slice(0, 10)) {
      ...
      // В тихом режиме (старт): только запоминаем, не отправляем
      if (silentMode) {
        sentArticles.add(key);
        continue;
      }
...
async function main() {
  ...
  // ВАЖНО: первый проход — тихий режим.
  // Все текущие статьи запоминаются без отправки,
  // чтобы при перезапуске не было дублей.
  console.log("[Init] Тихий прогон фидов для предотвращения дублей...");
  await checkAllFeeds(true);
  console.log("[Init] Готово. Начинаем мониторинг новых статей.");
