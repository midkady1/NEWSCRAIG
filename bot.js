...
async function postToX(text) {
  if (!X_API_KEY || !X_API_SECRET || !X_ACCESS_TOKEN || !X_ACCESS_TOKEN_SECRET) {
    console.log("[X] Ключи не настроены, пропускаем");
    return;
  }
  try {
    ...
    if (res.ok) {
      const data = await res.json();
      console.log(`[${new Date().toISOString()}] [X] Твит опубликован: ${data?.data?.id}`);
    } else {
      const err = await res.text();
      console.error(`[X] Ошибка ${res.status}: ${err}`);
    }
  } catch (err) {
    console.error("[X] postToX exception:", err.message);
  }
}

// Формирует английский текст твита (макс 280 символов)
function buildTweetText(titleEn, descEn, analyticsBlock, link, vip) {
  const tag   = vip ? "🔴 BREAKING" : "⚡ MACRO";
  // Убираем markdown-разметку из аналитики для X
  const analytics = analyticsBlock
    ? "\n\n" + analyticsBlock
        .replace(/\*([^*]+)\*/g, "$1")   // убрать **bold**
        .replace(/_([^_]+)_/g,   "$1")   // убрать _italic_
        .replace(/📊 \*?LLM Аналитика:?\*?/gi, "📊 Analysis:")
        .replace(/📊 \*?Аналитика:?\*?/gi,     "📊 Analysis:")
        .replace(/💡\s*/g, "💡 ")
    : "";

  const shortDesc = descEn.length > 120 ? descEn.slice(0, 117) + "…" : descEn;
  const descPart  = shortDesc ? `\n\n${shortDesc}` : "";

  let tweet = `${tag}\n\n📌 ${titleEn}${descPart}${analytics}\n\n🔗 ${link}`;

  // Обрезаем до 280 символов если не вмещается (без ссылки не режем)
  if (tweet.length > 280) {
    const linkPart = `\n\n🔗 ${link}`;
    const budget   = 280 - linkPart.length - 3;
    tweet = `${tag}\n\n📌 ${titleEn}${analytics}`.slice(0, budget) + "…" + linkPart;
  }
...
async function checkFeed(feedUrl, strict = false) {
  try {
    ...
    for (const entry of feed.items.slice(0, 10)) {
      ...
      // Постим в X полностью на английском
      const tweet = buildTweetText(titleEn, cleanDesc, analyticsBlock, link, vip);
      await postToX(tweet);
